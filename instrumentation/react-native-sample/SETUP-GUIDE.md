...
>This is an **Expo SDK 57** app. Development uses **Expo Go**. Splunk RUM instrumentation requires a **development build**
...
- Installs or **upgrades Expo Go** to match the current SDK (required after SDK upgrades)
- Starts Metro at `http://localhost:8081` with `--localhost` (uses `localhost` hostnames so iOS Simulator can connect on modern macOS)
- Registers the `exp://` URL handler and opens the project in Expo Go
---
